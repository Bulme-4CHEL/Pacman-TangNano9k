...
module video #(
    ...
    // FIX: Partitioned linebuffers (Distributed RAM)
    // 8 independent buffers x 16 depth x 4 bits.
    // Allows 8 parallel reads without huge Multiplexers (saves LUTs) 
    // and without port conflicts (fixes placement error).
    (* ram_style = "distributed" *) reg [3:0] sprite_linebuffer_0 [15:0];
    (* ram_style = "distributed" *) reg [3:0] sprite_linebuffer_1 [15:0];
    (* ram_style = "distributed" *) reg [3:0] sprite_linebuffer_2 [15:0];
    (* ram_style = "distributed" *) reg [3:0] sprite_linebuffer_3 [15:0];
    (* ram_style = "distributed" *) reg [3:0] sprite_linebuffer_4 [15:0];
    (* ram_style = "distributed" *) reg [3:0] sprite_linebuffer_5 [15:0];
    (* ram_style = "distributed" *) reg [3:0] sprite_linebuffer_6 [15:0];
    (* ram_style = "distributed" *) reg [3:0] sprite_linebuffer_7 [15:0];
    ...
    // FIX: Calculate write value combinatorially outside the always block
    // to fix syntax error "Local declaration in unnamed block".
    wire [3:0] write_val = (y[`YBITS-1:1] >= sprite_y_d2 && y[`YBITS-1:1] <  sprite_y_d2 + 16) ? color : 4'h0;
    ...
    always @(posedge clk) begin
        ...
        if(xs[`XBITS-1:7] == 0) begin
            // Demultiplex write to the correct buffer segment
            case(xs[6:4]) // Select sprite 0..7
                3'd0: sprite_linebuffer_0[xs[3:0]] <= write_val;
                3'd1: sprite_linebuffer_1[xs[3:0]] <= write_val;
                3'd2: sprite_linebuffer_2[xs[3:0]] <= write_val;
                3'd3: sprite_linebuffer_3[xs[3:0]] <= write_val;
                3'd4: sprite_linebuffer_4[xs[3:0]] <= write_val;
                3'd5: sprite_linebuffer_5[xs[3:0]] <= write_val;
                3'd6: sprite_linebuffer_6[xs[3:0]] <= write_val;
                3'd7: sprite_linebuffer_7[xs[3:0]] <= write_val;
            endcase
        end
    end
    ...
    always @(posedge clk) begin
        // FIX: Independent read from each partitioned buffer
        p0 <= (sx0 < 16) ? sprite_linebuffer_0[sx0[3:0]] : 4'h0;
        p1 <= (sx1 < 16) ? sprite_linebuffer_1[sx1[3:0]] : 4'h0;
        p2 <= (sx2 < 16) ? sprite_linebuffer_2[sx2[3:0]] : 4'h0;
        p3 <= (sx3 < 16) ? sprite_linebuffer_3[sx3[3:0]] : 4'h0;
        p4 <= (sx4 < 16) ? sprite_linebuffer_4[sx4[3:0]] : 4'h0;
        p5 <= (sx5 < 16) ? sprite_linebuffer_5[sx5[3:0]] : 4'h0;
        p6 <= (sx6 < 16) ? sprite_linebuffer_6[sx6[3:0]] : 4'h0;
        p7 <= (sx7 < 16) ? sprite_linebuffer_7[sx7[3:0]] : 4'h0;
    end
